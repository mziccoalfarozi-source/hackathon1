...
export default function PeriksaPasien() {
  ...
  const { user } = useAuth()
  const { getPatientById, assignDoctor, addDiagnosisAndPrescription } = useQueue()
  const patient = getPatientById(id || '')

  const [diagnosis, setDiagnosis] = useState('')
  const [prescriptions, setPrescriptions] = useState<Prescription[]>([
    { id: '1', medicationName: '', dosage: '', frequency: '', duration: '', notes: '' }
  ])
  ...
  if (!patient) {
    return (
      <div className="text-center py-20">
        ...
        <Button variant="outline" onClick={() => navigate('/dokter')} className="mt-4 gap-2">
  ...
  const handleAssign = () => {
    if (!user) return
    assignDoctor(patient.id, user.id, user.name)
    setIsAssigned(true)
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      addDiagnosisAndPrescription(patient.id, diagnosis, validPrescriptions)
      setIsSubmitting(false)
      toast.success('Pasien berhasil dirujuk ke farmasi!')
      setTimeout(() => navigate('/dokter'), 1000)
    }, 1500)
  }

  const getSymptomLabel = (val: string) => SYMPTOM_OPTIONS.find(s => s.value === val)?.label || val
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="sm" onClick={() => navigate('/dokter')} className="gap-2">
          <ArrowLeft className="w-4 h-4" />Kembali
        </Button>
      </div>
        ...
        <Card className="border-amber-200 bg-amber-50">
          <CardContent className="p-4 flex items-center justify-between">
            ...
            <Button onClick={handleAssign} className="bg-amber-600 hover:bg-amber-700 gap-2">
              <Stethoscope className="w-4 h-4" />Tangani Pasien
            </Button>
          </CardContent>
        </Card>
      )}
      ...
      <div className="grid lg:grid-cols-3 gap-6">
          ...
          <Card className="shadow-sm">
            <CardHeader className="pb-3">
              <CardTitle className="text-sm flex items-center gap-2">
                <User className="w-4 h-4 text-emerald-600" />Data Pasien
              </CardTitle>
            </CardHeader>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <Card className="shadow-sm">
            <CardHeader className="pb-3">
              <CardTitle className="text-sm flex items-center gap-2">
                <Brain className="w-4 h-4 text-violet-600" />AI Triage Reasoning
              </CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card className="shadow-sm">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <Stethoscope className="w-5 h-5 text-emerald-600" />Diagnosis & Resep Obat
              </CardTitle>
              <CardDescription>Isi diagnosis dan resep obat untuk dirujuk ke farmasi</CardDescription>
            </CardHeader>
            <CardContent className="space-y-5">
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <Button variant="outline" size="sm" onClick={addPrescription} disabled={needsAssignment} className="gap-1 text-xs">
                    ...
                    <div key={rx.id} className="bg-slate-50 rounded-xl p-4 border border-slate-200 relative">
                      ...
                      <div className="grid sm:grid-cols-2 gap-3">
                        <div className="space-y-1 sm:col-span-2">
                          <Label className="text-xs">Nama Obat <span className="text-red-500">*</span></Label>
                          <Input value={rx.medicationName} onChange={e => updatePrescription(rx.id, 'medicationName', e.target.value)}
                            placeholder="cth: Amoxicillin" className="h-9" disabled={needsAssignment} />
                        </div>
                        <div className="space-y-1">
                          <Label className="text-xs">Dosis <span className="text-red-500">*</span></Label>
                          <Input value={rx.dosage} onChange={e => updatePrescription(rx.id, 'dosage', e.target.value)}
                            placeholder="cth: 500mg" className="h-9" disabled={needsAssignment} />
                        </div>
                        <div className="space-y-1">
                          <Label className="text-xs">Frekuensi</Label>
                          <Input value={rx.frequency} onChange={e => updatePrescription(rx.id, 'frequency', e.target.value)}
                            placeholder="cth: 3x sehari" className="h-9" disabled={needsAssignment} />
                        </div>
                        <div className="space-y-1">
                          <Label className="text-xs">Durasi</Label>
                          <Input value={rx.duration} onChange={e => updatePrescription(rx.id, 'duration', e.target.value)}
                            placeholder="cth: 5 hari" className="h-9" disabled={needsAssignment} />
                        </div>
                        <div className="space-y-1">
                          <Label className="text-xs">Catatan</Label>
                          <Input value={rx.notes || ''} onChange={e => updatePrescription(rx.id, 'notes', e.target.value)}
                            placeholder="Setelah makan" className="h-9" disabled={needsAssignment} />
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              >
                {isSubmitting ? (
                  ...
                  <><Send className="w-4 h-4" />Selesai & Rujuk ke Farmasi</>
                )}
              </Button>
            </CardContent>
          </Card>
